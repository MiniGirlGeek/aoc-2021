class Bingo:
	...
	def __init__(self, string):
		...
		self.cols = [0, 0, 0, 0, 0]
		self.bingo = False
		self.last_bingo = False
		Bingo.number_boards += 1
		...
		for row in self.grid:
			...
			for num in row:
				if num == called_num:
					...
					self.rows[y] += 1
					self.cols[x] += 1
					if (self.rows[y] == 5 or self.cols[x] == 5) and self.bingo == False:
						self.bingo = True
						Bingo.total_bingos += 1
				x += 1
			y += 1
		print(Bingo.total_bingos, Bingo.number_boards)
		if Bingo.total_bingos == Bingo.number_boards:
			self.last_bingo = True
		return self.last_bingo
